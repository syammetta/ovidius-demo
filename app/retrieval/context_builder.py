...
Every stage is instrumented with OpenTelemetry spans. Each stage has graceful
fallback so a single failure doesn't crash the entire request.
"""
...
async def retrieve(
    ...
    with tracer.start_as_current_span("retrieve_pipeline") as pipeline_span:
        ...
        try:
            ...
            logger.warning("Classification stage failed: %s", e)
            cls_ms = 0

        if classification is None:
            classification = QueryClassification(
                intent="complex", topics=[], doc_types=[], sections=[],
                reasoning="classification unavailable",
            )
        if strategy is None:
            strategy = RetrievalStrategy(
                name="Deep Search", top_n=25, top_k=8,
        ...
        await progress("hybrid_search", "complete", {
            ...
            "vector_hits": hs_result.vector_count if hs_result else 0,
            "bm25_hits": hs_result.bm25_count if hs_result else 0,
            "both_hits": hs_result.both_count if hs_result else 0,
            "metadata_boosted": hs_result.metadata_boosted_count if hs_result else 0,
            "lanes": hs_result.lanes_used if hs_result else 0,
            "duration_ms": hs_ms,
        })

        if not candidates:
            empty_corrected = CorrectedRetrieval(
                chunks=[], confidence=RetrievalConfidence.LOW_CONFIDENCE,
                filtered_count=0, original_count=0,
            )
            return RetrievalResult(
                children=[], parent_contents={}, corrective=empty_corrected,
        ...
        try:
            ...
            logger.warning("Rerank failed, using raw hybrid results: %s", e)
        ...
        try:
            ...
            logger.warning("Corrective eval failed, passing all chunks: %s", e)
            corrected = CorrectedRetrieval(
                chunks=reranked,
        ...
        if corrected.confidence == RetrievalConfidence.LOW_CONFIDENCE and corrected.transformed_query:
            ...
            try:
                ...
                logger.warning("Query retry failed: %s", e)
        ...
        try:
            ...
            logger.warning("Parent fetch failed: %s", e)
            pf_ms = 0
